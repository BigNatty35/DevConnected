...
router.get(
  ...
  passport.authenticate('jwt', {session: false}), 
    ...
    Profile.findOne({user: req.user.id})
      ...
      .then(profile => {
      if(!profile) {
        errors.noprofile = "No profile is found for this user";
...
router.post('/', passport.authenticate('jwt', { session: false }), (req, res) => {
  ...
  if(typeof req.body.skills !== undefined) {
    profileFields.skills = req.body.skills.split(','); // split into an array of strings
  }
  ...
  // Look for users profile
  Profile.findOne({user: req.user.id})
  .then(profile => {
    if(profile) {
      //If User has an existiing profile, Update it with the new fields
      ...
      Profile.findOne({handle: profileFields.handle}).then(profile => {
        ...
        //If unable to find a profile with the handle, SAVE the Profile
...
//@desc remove the experience by id
//@access Private
router.delete(
  '/experience/:exp_id',
   passport.authenticate('jwt', {session: false}),
  ...
  Profile.findOne({user: req.user.id})
    .then(profile => {
      ...
      // Save the profile with deleted experience
      profile.save().then(profile => res.json(profile));
    })
    .catch(err => res.status(404).json(err));
});
...
//@desc remove the education by id
//@access Private
router.delete(
  '/education/:edu_id',
   passport.authenticate('jwt', {session: false}),
